...
Exposes a lightweight HTTP interface that the host-side Whisper service can call.
"""
...
def _init_logging() -> None:
    ...
    try:
        ...
        # Fall back to stderr only if the log directory is not writable.
        pass
...
_init_logging()
logger = logging.getLogger("aila.mind")


class VoiceAgent:
    """Coordinates dialogue with the host services."""
    ...
    def _speak(self, text: str) -> None:
        ...
        try:
            with urlopen(request, timeout=30):
                logger.debug("TTS request delivered to Piper.")
        except URLError as exc:
            logger.warning("Failed to reach Piper (%s): %s", url, exc)

    def _log_interaction(self, user_text: str, reply: str, snapshot: SenseSnapshot) -> None:
        logger.info(
            "turn completed user=%r reply=%r senses=%s",
            user_text,
            reply,
            snapshot.summary(),
        )
...
class AgentRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802 (BaseHTTPRequestHandler requires camelCase)
        if self.path == "/health":
